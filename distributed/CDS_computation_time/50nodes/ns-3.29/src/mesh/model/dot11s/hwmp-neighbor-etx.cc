 ...
 *
 * Adapted by:
 * Oscar Bautista <obaut004@fiu.edu>, <ogbautista@gmail.com>,
...
NeighborEtx::Print (std::ostream & os)
{
  ...
  //os << "currentTimeSlot=\"" << (uint32_t) m_lppTimeStamp << "\"" << std::endl;
  for (std::map<Mac48Address, Etx>::const_iterator i = m_neighborEtx.begin (); i != m_neighborEtx.end (); ++i)
    {
      ...
      //os << "<PeerLink peerAddress=\"" << i->first << "\" metric=\"" << linkMetric << "\"/>" << std::endl;
    ...
    //printf("current time slot= %d", m_lppTimeStamp);
